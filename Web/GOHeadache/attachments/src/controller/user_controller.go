...
func (u *UserController) ShowFlag(c *fiber.Ctx) error {
	config.RoleRights = config.AllRoles
	return c.Status(fiber.StatusCreated).
		JSON(response.Common{
			Code:    fiber.StatusCreated,
			Status:  "success",
			Message: fmt.Sprintf("Congrat sir, %s", config.Flag),
		})
}

func (u *UserController) ResetRoles(c *fiber.Ctx) error {
	if !c.IsFromLocal() {
		return fiber.NewError(fiber.StatusUnauthorized, "Invalid Not From Local: Unauthorized")
	}
	config.RoleRights = config.AllRoles
	return c.Status(fiber.StatusOK).
		JSON(response.Common{
			Code:    fiber.StatusCreated,
			Status:  "success",
			Message: fmt.Sprintf("success resetting: %+v", config.RoleRights),
		})
}
